...
textbooks. 
- Write a function GMM(data.matrix, K) which returns a list similar
  to the result of the stats::kmeans function. 
- for the initialization, you can use the cluster assignments from
  kmeans.
- Make a figure that compares the negative log likelihood (NLL) for
  the two kmeans algorithms. Plot NLL as a function of K (number of
