...
$$T_{ik} \gets \frac{\pi_k f(x_i, \mu_k, S_k)}{\sum_{k=1}^K \pi_k f(x_i, \mu_k, S_k)}$$.

In the M step we update the cluster parameters, 

- $\pi_k \gets \frac{1}{n}\sum_{i=1}^n T_{i,k}$,
- $\mu_k \gets \frac{\sum_{i=1}^k T_{i,k} x_i}{\sum_{i=1}^k T_{i,k}}$,
- $S_k \gets \frac{\sum_{i=1}^k T_{i,k} (x_i-\mu_k)(x_i-\mu_k)^\intercal}{\sum_{i=1}^k T_{i,k}}$ (no constraints).
...
- $S \gets \frac{\sum_{i=1}^k \text{Diag}[T_{i,k} (x_i-\mu_k)(x_i-\mu_k)^\intercal]}{\sum_{i=1}^k T_{i,k}}$ (diagonal constraint).
- $S_{k,k} = \sum_{i=1}^n T_{i,k} D_{i,k}^2$ where $D\in\mathbb
  R^{n\times p}$ is the difference matrix $X-\mu_k$. (avoids matrix
  multiplication, linear rather than quadratic time in feature
  dimension $p$)
