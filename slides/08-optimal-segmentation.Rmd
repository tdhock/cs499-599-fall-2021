...
\hline
1 & $O(n)$   & $O(n)$   & $O(n\log n)$--$O(n^2)$ & $O(n)$ \\
2 & $O(n^2)$ & $O(n^2)$ & $O(n\log n)$--$O(n^2)$ & $O(\log n)$--$O(n)$\\
3 & $O(n^3)$ & $O(n^2)$ & $O(n\log n)$--$O(n^2)$ & $O(\log n)$--$O(n)$\\
4 & $O(n^4)$ & $O(n^2)$ & $O(n\log n)$--$O(n^2)$ & $O(\log n)$--$O(n)$\\
$\vdots$ &     $\vdots$ &     $\vdots$  & $\vdots$ 
\end{tabular}
```

For example we have $\log n = `r log(N.data)`$, $n = `r N.data`$,
$n \log n = `r N.data*log(N.data)`$,
...
For computing best model in $S$ segments and $n$ data,

- Dynamic programming: $O(n S)$ loss $\mathcal L_{s,t}$ values to
  store.
- Binary segmentation: $O(S)$ split candidates to store.
