...
- Forward-backward sub-routine: computing the probability of being in
...
log.pi.dt <- data.table(
  data.i=-1, state=seq_along(log.pi.vec), log.prob=log.pi.vec)
...
gg.fwd <- function(state.k, data.t){
  some.tiles <- rbind(
    ...
    tile.dt[
    (state <= state.k & data.i == data.t) |
    data.i < data.t, .(data.i, state, log.prob)])
  seg.dt <- rbind(
    log.emission.dt[data.i==data.t, data.table(
      data.prev=data.i+0.3*c(-1,0,1),
      data.t,
      state.prev=state,
      state.k)],
    if(data.t>1)log.A.dt[to==state.k, data.table(
      data.prev=data.i,
      data.t,
      state.prev=state, 
      state.k)]
    else log.pi.dt[, data.table(
      data.prev=data.i,
      data.t,
      state.prev=state,
      state.k)],
    if(data.t>1)data.table(
      data.prev=data.t-1,
      data.t,
...
- When running the forward algorithm on large data (more than a few
  hundred time points), the log probability gets very small at the end
