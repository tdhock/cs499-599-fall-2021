...
chrom.dt[, i := data.i]
setkey(chrom.dt, i, data.i)
over.dt <- foverlaps(chrom.dt, segs)
...
gg <- ggplot()+
  ...
  geom_blank(aes(
    data.i, prob),
    data=data.table(xpanel="data.i", ypanel="probability", prob.dt))+
  geom_line(aes(
...
state.values <- c("#1B9E77", "#D95F02", "#7570B3","#E7298A")
gg <- ggplot()+
  scale_x_continuous(
    "Position/index in data sequence",
    breaks=seq(0, 1000, by=100))+
  ...
  geom_point(aes(
    data.i, logratio),
    data=data.table(ypanel="data values", pro.dt))+
  geom_line(aes(
    data.i, prob, color=state),
