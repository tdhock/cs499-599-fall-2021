...
# TODO simpler and more interesting Viterbi example
...
baum.welch.multiple <- function(it){
  ...
  ggplot()+
    ...
    geom_blank(aes(
      norm.density, y),
      data=data.table(ypanel="data values", chromosome=cfac("density"), dens.dt))+
    geom_tile(aes(
      x, y, fill=prob),
      data=data.table(chromosome=cfac("density"), ypanel="probability", it.trans.prior))+
    geom_text(aes(
    ...
    geom_text(aes(
      ...
      vjust=1.1,
      hjust=1,
      data=data.table(ypanel="data values", chromosome=cfac("density"), it.mean[1]))+
    scale_fill_gradient(low="white", high="red", limits=c(0,1))+
    geom_line(aes(
      data.i, prob, color=state),
    ...
    ## TODO add panels for density and initial/transition parameters.
    theme_bw()+
    theme(panel.spacing=grid::unit(0, "lines"))
}
```
