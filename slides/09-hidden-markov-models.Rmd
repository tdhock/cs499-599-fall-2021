...
- emission: define the probablity of observing data $y$ in state $k$,
  $b_j(y)\in[0,1]$ (mean, covariance).
...
mean.vec <- c(0.25, 0, -0.5)
n.states <- length(mean.vec)
y.vec <- chrom.dt[seq(1, .N, by=4), logratio]
A.mat <- matrix(1/n.states, n.states, n.states)
sd.param <- 1
## Need to compute emission whenever parameters are updated.
N.data <- length(y.vec)
log.emission.mat <- dnorm(
  ...
  log=TRUE)
pi.vec <- rep(1/n.states, n.states)
str(list(mean=mean.vec, sd=sd.param, transition=A.mat, initial=pi.vec))
viterbi.list <- plotHMM::viterbi_interface(log.emission.mat, A.mat, pi.vec)
some.dt <- data.table(
...
while(!done){
  ...
  A.mat <- plotHMM::transition_interface(
...
while(!done){
  ...
  (pi.vec <- colMeans(prob.mat[first.row.vec,]))
...
baum.welch.multiple <- function(it){
  ...
  dput(RColorBrewer::brewer.pal(4, "Dark2"))
  state.values <- c(
    "#E7298A","#7570B3",
    "#1B9E77", "#D95F02")
  ggplot()+
    ...
    ylab("")+
    scale_color_manual(values=state.values)+
    scale_fill_manual(values=state.values)+
    geom_text(aes(
      Inf, Inf, label=sprintf(
        "it=%d log(lik)=%.4f", it, log.lik)),
      hjust=1,
      vjust=1.1,
      data=data.table(
        it.lik,
        ypanel="data values",
        chromosome=factor("1", levels(it.prob$chromosome))))+
    geom_hline(aes(
      yintercept=mean),
      size=2,
...
# Time/space complexity TODO

---

# Possible exam questions TODO
