...
- The values in the weight matrices $W_l$ are the model parameters
  which are learned using gradient descent.
- The gradient descent learning algorithm starts with arbitrary/random
  weight matrices $W_l$ close to zero.
- Each iteration of gradient descent updates the weight matrices $W_l$
  in order to get better predictions (lower mean squared
  reconstruction error).
- The batch size is the number of observations for which the gradient
  is computed and summed during each iteration.
...
if(file.exists(m.cache.rds)){
  ...
  code_mat <- predict(intermediate_layer_model, X.mat)
  mnist.results <- list(
    code=code_mat,
...
- To compute low-dimensional embedding for an image $x$ we compute:
